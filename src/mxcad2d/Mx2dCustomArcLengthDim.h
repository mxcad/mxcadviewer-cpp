...
class Mx2dCustomArcLengthDim : public Mx2dCustomAnnotation
{
	ACRX_DECLARE_MEMBERS(Mx2dCustomArcLengthDim);
public:
	Mx2dCustomArcLengthDim(void);
	Mx2dCustomArcLengthDim(const McGePoint3d& startPt, const McGePoint3d& midPt, const McGePoint3d& endPt, const McGePoint3d& dimPt, double textHeight);

	virtual ~Mx2dCustomArcLengthDim(void);
...
public:
	...
	void setMidPt(const McGePoint3d& pt);
...
protected:
	...
	McDbArc* createArc() const;

	McDbText* createText() const;

	McDbLine* createExtLine1() const;
	McDbLine* createExtLine2() const;

	McDbLine* createObliqueLine1() const;
	McDbLine* createObliqueLine2() const;

	void calculateArc(McGePoint3d& center, double& radius, double& length) const;
