...
class Mx2dCustomCartesianCoord : public Mx2dCustomAnnotation
{
	ACRX_DECLARE_MEMBERS(Mx2dCustomCartesianCoord);
public:
	Mx2dCustomCartesianCoord(void);
	Mx2dCustomCartesianCoord(const McGePoint3d& startPt, const McGePoint3d& endPt, double textHeight);

	virtual ~Mx2dCustomCartesianCoord(void);
...
protected:
	McDbVoidPtrArray createTexts(double& len) const;
	McDbPolyline* createPoly(double len) const;
