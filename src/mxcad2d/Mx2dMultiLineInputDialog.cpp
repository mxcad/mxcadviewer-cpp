...
Mx2dMultiLineInputDialog::Mx2dMultiLineInputDialog(QWidget* parent)
	: QDialog(parent)
{
...
Mx2dMultiLineInputDialog::~Mx2dMultiLineInputDialog()
{
}

QString Mx2dMultiLineInputDialog::getText() const
{
	return m_text;
}

void Mx2dMultiLineInputDialog::setupUi()
{
	setWindowTitle(tr("Multi-line Text"));
...
void Mx2dMultiLineInputDialog::setupConnections()
{
	// Connect text edit box content change signal to onTextChanged slot
	connect(m_pTextEdit, &QTextEdit::textChanged, this, &Mx2dMultiLineInputDialog::onTextChanged);

	// Connect insert button click signal to onInsertClicked slot
	connect(m_pInsertButton, &QPushButton::clicked, this, &Mx2dMultiLineInputDialog::onInsertClicked);
}

void Mx2dMultiLineInputDialog::onTextChanged()
{
...
void Mx2dMultiLineInputDialog::onInsertClicked()
{
	...
	// Execute command to insert multi-line text
	std::string sAscii = m_text.toLocal8Bit().constData(); // Content
	MrxDbgRbList spParam = Mx::mcutBuildList(RTSTR, sAscii.c_str(), 0);
	Mx2d::execCmd2d(MxUtils::gCurrentTab, "Mx_DrawMText", spParam.orphanData());
}